...
-- Team Name : The Analytix 
-----------------------------------------------
-- Task1 - Build Data Model, Data Cleaning and Preprocessing
-----------------------------------------------

-- Display all rows ordered by RowID in ascending order
SELECT * 
FROM SuperStore
ORDER BY RowID ASC;
...
SELECT 
    ...
    (SELECT COUNT(COLUMN_NAME) 
     FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_NAME = 'SuperStore') AS Total_Columns;

-- Identify duplicate rows using a Common Table Expression (CTE)
...
SELECT * 
FROM DuplicateRows_CTE 
WHERE rn > 1;

-- Retrieve specific rows based on their RowID
SELECT * 
FROM SuperStore
...
-- Handle missing values by replacing them with median or average values
...
-- Convert Quantity column data type to integer
ALTER TABLE SuperStore 
ALTER COLUMN Quantity INT;

-- Convert Order Date and Ship Date columns to date format
ALTER TABLE SuperStore 
...
-- Display column information after modifications
SELECT COLUMN_NAME, DATA_TYPE 
FROM INFORMATION_SCHEMA.COLUMNS 
WHERE TABLE_NAME = 'SuperStore';
